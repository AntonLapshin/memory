...
export function mergeMemoryFile(
  existingPath: string,
  newData: WriteMemoryData,
): MemoryFile {
  const now = new Date().toISOString();
  ...
  const existing = readMemoryFile(normalized);
  if (!existing) {
    throw new Error(`Cannot merge: existing file not found at ${existingPath}`);
  }
  ...
  const summary = newData.summary || existing.summary;
  const title = newData.title || existing.title;
  ...
  let existingBody = existing.content.trim();
  const relatedSplit = existingBody.split(/\n## Related\b/);
  if (relatedSplit.length > 1) {
    existingBody = relatedSplit[0].trimEnd();
  }

  const body = existingBody + appendBlock;

  const fullBody = relatedSection
    ? body +
      '\n\n## Related\n<!-- Auto-generated by memory tool. Do not edit manually. -->\n' +
      relatedSection
    : body +
  ...
  const fullContent = frontmatter + '\n\n' + fullBody + '\n';
  fs.writeFileSync(fullPath, fullContent, 'utf-8');
  logger.info('Merged into memory file', { path: normalized, title });
