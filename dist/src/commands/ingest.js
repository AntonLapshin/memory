...
export async function ingestCommand(rawText, options = {}) {
    ...
    if (options.path)
        summary.suggested_path = options.path;
    if (options.tags)
        summary.tags = options.tags.split(',').map((t) => t.trim());
    ...
    if (!options.noCrossRef) {
        ...
        const relatedMemories = relatedSearches.filter((r) => r.path !== memory.path && r.score > 0.4);
