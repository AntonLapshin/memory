...
Return ONLY valid JSON, no other text.`;
    const response = await chat([{ role: 'user', content: prompt }], { json: true, temperature: 0.3 });
    ...
    try {
        ...
        throw new Error(`Failed to parse LLM response as JSON. Response: ${response.substring(0, 500)}`);
    }
}
...
Respond with JSON: {"link": true/false, "reason": "brief reason"}`;
    const response = await chat([{ role: 'user', content: prompt }], { json: true, temperature: 0.1 });
    try {
